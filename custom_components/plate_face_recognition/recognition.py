"""Core recognition logic for license plates and faces."""
...
def _get_ocr_reader():
    ...
    if _ocr_reader is None:
        try:
            ...
            _LOGGER.debug("EasyOCR reader initialised.")
...
def _detect_plate_candidates(img_bgr: np.ndarray) -> list[np.ndarray]:
    ...
    # Sort by area, biggest first; consider top candidates
    ...
    for cnt in contours:
        ...
        # Typical plate aspect ratio: 2:1 to 6:1; not too small, not too large
        if 1.5 < aspect < 7.0 and 0.005 < area_ratio < 0.3:
            roi = img_bgr[y : y + h, x : x + w]
            candidates.append(roi)

    # Always include a resized full image as a fallback
