"""Face database management – standalone version for the HA Add-on.

Stores face encodings in a pickle file on disk (path configurable via
the PFR_DATA_DIR environment variable, defaults to /data).
...
# Default data directory – overridden by PFR_DATA_DIR env var (set by run.sh)
_DEFAULT_DATA_DIR = Path(os.environ.get("PFR_DATA_DIR", "/data"))
DEFAULT_DB_PATH = _DEFAULT_DATA_DIR / "known_faces.pkl"
...
class FaceManager:
    ...
    def add_face(self, name: str, encodings: list[np.ndarray]) -> int:
        """
        Add *encodings* for *name*. Existing encodings are kept.
    ...
    def delete_face(self, name: str) -> bool:
        """Remove all encodings for *name*. Returns True if found."""
